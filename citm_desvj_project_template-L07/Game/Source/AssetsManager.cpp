...
{
	// Determine if the PhysicsFS library is initialized, we can check it to avoid errors.
	if (PHYSFS_isInit())
		printf("Asset Manager is succefully loaded");
	else
		printf("Failed loading Asset Manager");
...
uint AssetsManager::Load(const char* path, char** buffer) const
{
	...
	if (!PHYSFS_eof(file))
	{
		...
		*buffer = new char[lenght];
		...
		uint bytes = PHYSFS_read(file, *buffer, 1, lenght);


		if (bytes != lenght)
		{
			printf("%s", path, "ERROR: %s", PHYSFS_getLastError());
